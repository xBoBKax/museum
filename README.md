...
Это новая версия сайта `museum.eriskip.com` на React + Vite + React Router. Проект не использует WordPress, PHP-темы, плагины или WordPress API: после сборки получается статический сайт из папки `dist/` с многостраничной навигацией через SPA fallback.

## Почему выбран React + Vite

- сайт можно развернуть как статические файлы на Nginx, Apache2 или IIS;
- вместо якорного одностраничника используются полноценные маршруты `/about`, `/news`, `/collection`, `/media`, `/contacts`;
...
│   ├── main.jsx               # React Router, страницы и компоненты сайта
...
- `/` — главная страница с корпоративным hero-блоком, статистикой, кратким описанием, экспонатами и новостями.
- `/about` и совместимость со старым `/o-muzee` — история музея, направления работы и данные о пространстве.
- `/news` и совместимость со старым `/novosti` — отдельная страница новостей.
- `/collection`, `/kollekcija`, `/kollekciya` — каталог с поиском, фильтрами и карточками.
- `/collection/:slug` — подготовленная детальная страница экспоната.
- `/media` и совместимость со старым `/videomaterialy` — видеоматериалы, виртуальный тур, библиотека проектов.
- `/contacts` — адрес, email, сайт ЭРИС и Vk.
- Внешние переходы: Яндекс.Диск для проектов/наград и существующий Unity/WebGL-тур.

## Улучшения относительно старой версии

- Корпоративная палитра ЭРИС: синий, желтый акцент, белые промышленные карточки и строгая сетка.
- Многостраничная навигация вместо якорей одной страницы.
- Современный адаптивный интерфейс для ПК и телефонов.
- Sticky header и полноценное мобильное меню.
- Рабочие внутренние ссылки на страницы и карточки экспонатов.
